...
from sim.mahasiswa.forms import mahasiswa_F, loginmahasiswa_F, editmahasiswa_F, pengaduan_F, editpengaduan_F,  editinformasi_F, informasi_F
from sim.models import Tmahasiswa, Tpengaduan, Tinformasi
...
@rmahasiswa.route("/pengaduan/<int:ed_id>/detail_pengaduan", methods=['GET', 'POST'])
@login_required
...
@rmahasiswa.route("/informasi", methods=['GET', 'POST'])
@login_required
def informasi():
    dt_informasi=Tinformasi.query.filter_by(mahasiswa_id=current_user.id)
    form=informasi_F()
    if form.validate_on_submit():
        #tambah data informasi
        add_informasi= Tinformasi(subjek=form.subjek.data, kategori=form.kategori.data, detail_informasi=form.detail_informasi.data, mahasiswa=current_user)
        db.session.add(add_informasi)
        db.session.commit()
        flash('Data informasi berhasil di Tambahkan','warning')
        return redirect(url_for('rmahasiswa.informasi'))
    return render_template('informasi.html', form=form, dt_informasi=dt_informasi)


@rmahasiswa.route("/informasi/<int:ed_id>/update", methods=['GET', 'POST'])
@login_required
def update_informasi(ed_id):
    form=editinformasi_F()
    dt_informasi=Tinformasi.query.get_or_404(ed_id)
    if request.method=="GET":
        form.subjek.data=dt_informasi.subjek
        form.kategori.data=dt_informasi.kategori
        form.detail_informasi.data=dt_informasi.detail_informasi

    elif form.validate_on_submit():
        dt_informasi.subjek=form.subjek.data
        dt_informasi.kategori=form.kategori.data
        dt_informasi.detail_informasi=form.detail_informasi.data
        db.session.commit()
        flash('Data Berhasil di Ubah', 'warning')
        return redirect(url_for('rmahasiswa.informasi'))
    return render_template('edit_informasi.html', form=form)
...
@rmahasiswa.route("/informasi/<int:ed_id>/detail_informasi", methods=['GET', 'POST'])
@login_required
def detail_informasi(ed_id):
    dt_informasi=Tinformasi.query.get_or_404(ed_id)
    return render_template('detail_informasi.html', dt_informasi=dt_informasi)
