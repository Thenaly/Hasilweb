...
class Tmahasiswa(db.Model, UserMixin):
    ...
    informasis = db.relationship('Tinformasi', backref='mahasiswa')
...
class Tinformasi(db.Model):
    ...
    subjek = db.Column(db.String(100), nullable=False)
    kategori = db.Column(db.String(50), nullable=False)
    detail_informasi = db.Column(db.String(300), nullable=False)
    tgl_baca = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    mahasiswa_id = db.Column(db.Integer, db.ForeignKey('tmahasiswa.id'))

    def __repr__(self):
        return f"Tinformasi('{self.subjek}','{self.kategori}','{self.detail_informasi}','{self.tgl_baca}')"